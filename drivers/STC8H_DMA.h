...
#ifndef	__STC8H_DMA_H
#define	__STC8H_DMA_H
...
#define SET_LCM_DMA_LEN(n)			DMA_LCM_AMT = (n)
...
typedef struct
{
	...
	u8	DMA_TX_Length;			//DMA传输总字节数  	(0~255) + 1, 不要超过芯片 xdata 空间上限
	u16	DMA_TX_Buffer;			//发送数据存储地址

	u8	DMA_RX_Enable;			//DMA使能  	ENABLE,DISABLE
	u8	DMA_RX_Length;			//DMA传输总字节数  	(0~255) + 1, 不要超过芯片 xdata 空间上限
	u16	DMA_RX_Buffer;			//接收数据存储地址
