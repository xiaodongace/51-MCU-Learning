...
/*
 * 公共非阻塞消抖状态。
 * raw保存最近一次采样值，stable保存已确认状态，change_ms记录raw变化时刻，
 * press_event锁存一次有效按下事件，直到业务调用Key_GetPressEvent()取走。
 */
 ...
 * 初始原始状态与稳定状态保持一致，避免把上电时的电平误判为一次按下事件。
 */
static void Key_EventInit(void) {
    u8 i;
    u16 now = Timers_GetSystemMs();

    /* 三个按键分别保存自己的状态和最近变化时间，互不影响。 */
...
/*
 * 读取并消费指定按键的一次性按下事件。
 * 返回1表示取得一个新事件；事件读取后立即清零，按住按键不会重复触发。
 */
 ...
 * 查询按键当前是否被按下。
 *
 * KEY1 和 KEY2 使用带上拉输入，未按下时引脚读到高电平（UP），按下时
 * 引脚被接到地而读到低电平（DOWN）。因此这里将“读到 DOWN”转换成对
 * 外部更易理解的布尔结果：非零为按下，0 为松开。
 *
 * 该函数故意不加入 delay_ms()：它可能在主循环中被频繁调用，消抖和
 * “只响应一次”的策略应由具体业务决定。
 */
